...
export default function Index() {
  ...
  const addEvent = () => {
    const newEvent = `${new Date().toLocaleTimeString()} - Battery ${Math.round(
      (level ?? 0) * 100,
    )}%`;
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Expo Battery Monitor</Text>

      <View style={styles.card}>
        <Text style={styles.label}>Battery Level</Text>

        <Text style={styles.value}>
          {level !== null ? `${Math.round(level * 100)}%` : "Unknown"}
        </Text>
      </View>
      ...
      <Button title="Add Battery Event" onPress={addEvent} />

      <View style={styles.eventBox}>
        <Text style={styles.eventTitle}>Battery Events</Text>
